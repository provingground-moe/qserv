...
void reportRequestState (COLLECTION const& requests,
    ...
    for (const auto &ptr: requests) {
        os  << " "   << std::setw(36) <<                                     ptr->id()
            ...
            << " | " << std::setw(11) << replica_core::Request::state2string(ptr->state())
            << " | " << std::setw(20) << replica_core::Request::state2string(ptr->extendedState())
            << " | " << std::setw(24) << replica_core::status2string(        ptr->extendedServerStatus())
            << "\n";
    }
    os  << "--------------------------------------+----------------------+--------+-------------+----------------------+--------------------------\n"
        << std::endl;
