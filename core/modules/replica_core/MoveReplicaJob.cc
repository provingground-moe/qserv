...
LOG_LOGGER _log = LOG_GET("lsst.qserv.replica_core.MoveReplicaJob");
    ...
    while (true) {
        ...
        if (numLaunched == numFinished) {
            if (errorReport && numLaunched - numSuccess)
                replica_core::reportRequestState (_replicationRequests, os);
            break;
        }
    }
    while (true) {
        ...
        if (numLaunched == numFinished) {
            if (errorReport && numLaunched - numSuccess)
                replica_core::reportRequestState (_deleteRequests, os);
            break;
        }
    }
}
