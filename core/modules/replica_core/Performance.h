 ...
 */
#ifndef LSST_QSERV_REPLICA_CORE_PERFORMANCE_H
#define LSST_QSERV_REPLICA_CORE_PERFORMANCE_H
...
/**
 * Worker-side class with performance counters of a request.
 *
 * All time counters are expressed in milliseconds since Epoch.
 * Undefined values are set to 0.
 */
...
public:
    ...
    /// Copy c-tor
    WorkerPerformance (const WorkerPerformance &p);

    /// Assignment operator
    WorkerPerformance & operator= (const WorkerPerformance &p);  

    /// Destructor
    virtual ~WorkerPerformance ();
...
private:
    ...
    /// Set internal state from the one of another object
    void setFrom (const WorkerPerformance &p);
...
#endif // LSST_QSERV_REPLICA_CORE_PERFORMANCE_H